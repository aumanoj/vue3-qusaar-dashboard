    ...
    <table class="table">
      ...
      <tbody v-if="side_menus!=null">
        <tr v-for="(sidemenu,key) in side_menus" :key="key">
          <td>{{key + 1}}</td>
          <td>{{sidemenu.title}}</td>
          <td>{{sidemenu.url}}</td>
          <td>{{sidemenu.position}}</td>
          <td>{{sidemenu.category.name}}</td>
          <td>
            <router-link :to="'/side-menu/'+sidemenu.id" class="text-black"><q-btn  size="10px" color="info" padding="8px" icon="fas fa-eye" class="q-mr-sm" @click="formModal = true , createOrEdit = false, editForm(sidemenu.id)" /></router-link>
        ...
        <q-card-section class="q-pt-none">
          <div class="row">
            <div class="col-12 q-mb-md">
                <select v-model="form.category_id" class="form-control" @click="subCategories($event)">
...
export default {
  ...
  computed:{
      ...mapGetters({
        side_menus:'sidemenu/side_menus',
        categories:'sidemenu/categories',
        paginate:'sidemenu/paginate',
  ...
  methods: {
    ...mapActions({
      ...
      edit:'sidemenu/edit',
      update:'sidemenu/update',
      delete:'sidemenu/delete',
    }),
    formReset(){
  ...
  subCategories(e){
      ...
      if(value.id == e.target.value){
        subCat.push(value.sub_category);
      } 
    });
      this.sub_categories = subCat[0];
    }
  }
}
</script>

    ...
    <table class="table">
      ...
      <tbody v-if="side_menus!=null">
        <tr v-for="(sidemenu,key) in side_menus" :key="key">
          ...
          <td>{{sidemenu.name}}</td>
          <td>
            <q-btn size="10px" color="info" padding="8px" icon="fas fa-eye"  @click="formModal = true , createOrEdit = false, editForm(sidemenu.id)" />
...
export default {
  ...
  computed:{
      ...mapGetters({
        ...
        sub_categories:'sidemenu/sub_categories',
